...
状态（2026-09-11）：本文保留原目标模型。时钟、库存、室内生活、日程，以及事件档案和首批观察者知识/长期倾向均已有实现；不能再按“阶段2尚未开始”判断进度。当前实现边界见文档14、16、18、19，目标中的全部表和通用双图尚未完成。
...
已确认并完成：人物内部状态采用正向“饱食度”（`satiety`），范围为0到100；100最舒适，随世界时间下降，进食后上升。旧历史记录不重写。
