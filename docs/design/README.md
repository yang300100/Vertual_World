...
| D005 | 每现实一分钟进行一次轻量状态心跳 | 已确认待实现 | `01-runtime-and-time.md` |
| D006 | 世界时间比例可在运行时修改，按实际现实时间差推进 | 已确认待实现 | `01-runtime-and-time.md` |
| D007 | 模型按累计世界时间或重要事件触发，不随每次心跳调用 | 已确认待实现 | `01-runtime-and-time.md` |
