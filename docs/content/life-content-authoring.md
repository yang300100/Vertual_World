...
- `resource_location_id` 和 `resource_key` 要同时填写或同时为 null。
- `resource_owner_id` 为 null 表示可合法公共收取；不是“主人暂时没想好”。
- 同地点的同一资源键只能定义一套来源规则，不能用不同商品重复增长同一资源池。
- `initial_resource` 只在该资源键原本不存在时使用，不覆盖旧储量；容量不得低于旧储量。
