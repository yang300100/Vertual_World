...
`动作：`（也支持英文冒号和 `行动：`）优先进入本地执行规则，不会被当前对话对象
强制转换成台词。休息、进食、移动、拾取、装备、工作、攻击复用原有动作规则；
记录、观察、誊清、核对会生成持久化的行动工作记录，显示在“笔记本 → 行动工作记录”。
记录保存来源事件、NPC 原始要求、实际地点、结果及部分完成状态，并消耗对应精力。
原图、星位测量或未走过路段缺少依据时保留待补全状态，不会凭一句输入宣告全部完成。

执行后，实际在场的原对话 NPC 才会收到已结算结果，并由模型生成下一步回应。
短期对话、旧对话召回及人物记忆均区分“玩家行动”和“玩家发言”。模型暂时失败时，
动作结果仍保留，可点击“只重试 NPC 回应”，不会重复执行动作或再次消耗精力。
未覆盖的动作会明确报告尚未执行，不会随机改成工作、攻击或交谈。
